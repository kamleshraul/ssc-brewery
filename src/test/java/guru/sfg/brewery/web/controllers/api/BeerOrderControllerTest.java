...
//    class createOrderTests {
    ...
    void createOrderUserNOTAuthCustomer() throws Exception {
    	//we are sending authentication for KEYWEST_USER but send cutomer id of STPETERCUSTOMER
    ...
    void pickUpOrderNotAdminUser() throws Exception {
    	//Non-Customer admin and customer role users should be forbidden to pickup order
    	BeerOrder beerOrder = stPeteCustomer.getBeerOrders().stream().findFirst().orElseThrow();
    ...
    void pickUpOrderCustomerUserAUTH() throws Exception {
    	// Authenticate STPETE_USER can pickup it's own order
        BeerOrder beerOrder = stPeteCustomer.getBeerOrders().stream().findFirst().orElseThrow();
    ...
    void pickUpOrderCustomerUserNOT_AUTH() throws Exception {
    	//Authententicate DUNEDIN user cannot access StPeteCustomer orders
    	BeerOrder beerOrder = stPeteCustomer.getBeerOrders().stream().findFirst().orElseThrow();

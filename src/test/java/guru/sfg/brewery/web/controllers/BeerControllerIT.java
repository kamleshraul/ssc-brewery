...
public class BeerControllerIT extends BaseIT {
	...
	class InitNewForm {
		...
		void initCreationFormAuth(String user, String pwd) throws Exception {
			...
			mockMvc.perform(get("/beers/new").with(httpBasic(user, pwd))).andExpect(status().isOk())
					.andExpect(view().name("beers/createBeer")).andExpect(model().attributeExists("beer"));
		}
	 ...
	 * @DisplayName("Init New Form")
	 * 
	 * @Nested class InitNewForm{
	 * 
	 * void initCreation }
	 * 
	 * 
	 * 
	 * @Test void initCreationFormBcrypt15() throws Exception{
	 ...
	 * @WithMockUser("spring")
	 * 
	 * @Test void findBeers() throws Exception{ mockMvc.perform(get("/beers/find"))
	 * .andExpect(status().isOk()) .andExpect(view().name("beers/findBeers"))
	 * .andExpect(model().attributeExists("beer")); }
	 * 
	 * @Test void findBeersHttpBasic() throws Exception{
	 * mockMvc.perform(get("/beers/find").with(httpBasic("spring", "security")))
	 * .andExpect(status().isOk()) .andExpect(view().name("beers/findBeers"))
	 * .andExpect(model().attributeExists("beer")); }
	 * 
	 * @Test void findBeersBySkippingHttpBasicAuth() throws Exception{
	 * mockMvc.perform(get("/beers/find")) .andExpect(status().isOk())
	 * .andExpect(view().name("beers/findBeers"))
	 * .andExpect(model().attributeExists("beer")); }
	 * 
	 * @Test void findBeersBySkippingHttpBasicAuthAnonymous() throws Exception{
